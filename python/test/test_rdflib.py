...
for index, row in pd_reader.iterrows():
    ...
    g.add((URIRef(ppr+row["Paper"]), URIRef(schema+"reviewedBy"), URIRef(conf+row["Reviewer1"])))
    g.add((URIRef(ppr+row["Paper"]), URIRef(schema+"reviewedBy"), URIRef(conf+row["Reviewer2"])))
